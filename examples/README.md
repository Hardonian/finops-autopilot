...
### jobforge/input.json
Inputs for the `finops analyze` JobForge integration command.

### output/jobforge/
Stable output artifacts generated by `finops analyze --stable-output`.
...
# Run JobForge analyze (stable mode)
./dist/cli.js analyze \
  --inputs examples/jobforge/input.json \
  --tenant demo-tenant \
  --project demo-project \
  --trace trace-demo \
  --out examples/output/jobforge \
  --stable-output
```
